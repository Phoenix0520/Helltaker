...
void Texture::UpdateColorBuffer(int path)
{
	...
	shader->UpdateColorBuffer(color, path, 0.0f, 0.0f, 0.0f);
}
...
{
	shader->UpdateColorBuffer(color, path, Time, 0.0f, 0.0f);
}
...
void Texture::CreateMultiShaderResourceView(wstring imageFile)
{
	...
	shader->SetShaderMultiResourceView(spriteResource->srv);
	spriteResources.insert(make_pair(imageFile, spriteResource));
}
