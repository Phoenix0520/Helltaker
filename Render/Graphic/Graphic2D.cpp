...
void Graphic2D::Merge()
{
	...
	Gdiplus::Bitmap* bit = new Gdiplus::Bitmap(536, 748, &graphic); // Bitmap 이미지 생성
	...
	for (int i = 1; i <= 4; i++)
	{
		wstring str = L"./Images/Dialogue/Lucifer/Test.png"; // (" + to_wstring(i) + L").png";
		images[0] = new Gdiplus::Image(str.c_str());
		memG->DrawImage(images[0], 0, 0, images[0]->GetWidth(), images[0]->GetHeight());

		str = L"./Images/Dialogue/Lucifer/Face (" + to_wstring(i) + L").png";
		images[i] = new Gdiplus::Image(str.c_str());
		memG->DrawImage(images[i], 175, 211, images[i]->GetWidth(), images[i]->GetHeight());

		//x += 175;

		SAFE_DELETE(images[i]);

		wstring writeFileName = L"./Images/Dialogue/Lucifer/Smile (" + to_wstring(i) + L").png";
